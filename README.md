...
│  └─────────────┘ └─────────────┘ └──────────┘ └───────┘ └───────────┘  │
│  ┌──────────────┐ ┌──────────┐ ┌────────────────────────────────────┐     │
│  │  solo-ui     │ │ langfuse │ │ agentgateway-config (wave7)        │     │
│  │  (wave6)     │ │ (wave6)  │ │  gateway/backends/routes/policies  │     │
│  │              │ │          │ │  + otel-fanout-collector + ESO     │     │
│  └──────────────┘ └──────────┘ └────────────────────────────────────┘     │
└──────────────────────────────────────────────────────────────────────────┘
  Vault stores all secrets → ESO syncs them → K8s Secrets → AgentGateway / Langfuse
  Gateway traces → otel-fanout-collector → Solo UI (ClickHouse) + Langfuse (OTLP/HTTP)
```
...
| OTel Fan-out Collector | agentgateway-system | `otel-fanout-collector` | Running |
| **Langfuse** | langfuse | `langfuse-web`, `-worker` + postgres/clickhouse/redis/minio/zookeeper | Running |
...
**Langfuse login:** email `sebastian.maniak@solo.io`. The password is generated at
deploy time and stored only in Vault — retrieve it with:

```bash
kubectl exec -n vault vault-0 -- vault kv get -field=init-user-password agentgateway/langfuse/config
```

> Langfuse credentials (DB passwords, encryption key, init API keys) are generated by
> `scripts/configure-vault.sh` and synced into the cluster by External Secrets — nothing
> sensitive is stored in Git. NodePort serves plain HTTP, so treat it as lab-only access.
...
│   ├── policies/
│   │   ├── tracing.yaml               # Distributed tracing → otel-fanout-collector (Solo + Langfuse)
